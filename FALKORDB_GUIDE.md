...
│   ├── repository.py        # Data access layer (не використовується напряму)
...
### Створення вузла
1. Виберіть таб "Node"
2. Введіть Label (наприклад: `Person`)
3. Введіть Properties у JSON форматі:
```json
{
  "name": "Alice",
  "age": 28,
...
2. Введіть Cypher запит або виберіть приклад
3. Натисніть "Execute Query"
4. Перегляньте результати в центральній секції
